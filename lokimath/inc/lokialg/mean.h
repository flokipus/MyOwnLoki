 ...
 * потери точности (менее печально, если mean_sum_and_divide).
 *
 * Данный метод поддерживает две стратегии:
 *      DivideAndAverage ~-- рекурсивно считает среднее двух соседних элементов; подходит в случае, если
 *      элементов много и возможно переполнение; использует дополнительную память;
 * и
 *      SumAndDivide ~-- складывает все и делит на число элементов; не использует доп. память;
 */
